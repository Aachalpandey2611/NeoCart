        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <motion.div variants={slideUp} initial="hidden" animate="visible" className="w-full md:w-1/3 lg:w-1/4">
            <div className="glass-dark rounded-3xl p-8 border border-white/10 shadow-2xl sticky top-24 pb-12 flex flex-col items-center">
              ...
              <h2 className="text-2xl font-bold text-white mb-2 text-center truncate w-full px-2">{user.name}</h2>
              <p className="text-gray-400 text-sm flex items-center gap-2 mb-6 bg-white/5 py-1.5 px-3 rounded-full border border-white/10 truncate max-w-full"><FiMail className="shrink-0" /> <span className="truncate">{user.email}</span></p>
              ...
              <div className="w-full space-y-3 mt-4 pt-8 border-t border-white/10">
                ...
                >
                  <span className="flex items-center gap-3"><FiUser className={activeTab === 'account' ? 'text-purple-400' : ''} /> Account Details</span>
                </button>
                <button 
                ...
                >
                  <span className="flex items-center gap-3"><FiPackage className={activeTab === 'orders' ? 'text-cyan-400' : ''} /> My Orders</span>
                  <span className={`px-2 py-0.5 rounded text-xs ${activeTab === 'orders' ? 'bg-cyan-500/30 text-white' : 'bg-white/10 text-gray-500'}`}>{orders.length}</span>
                </button>
              </div>
            </div>
          </motion.div>
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'orders' ? (
                <motion.div key="orders" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  ...
                  {loading ? (
                    ...
                    <motion.div variants={slideUp} initial="hidden" animate="visible" className="glass rounded-3xl p-12 text-center border-white/5">
                      ...
                      <button onClick={() => window.location.href='/products'} className="btn-primary mt-6">Shop Now</button>
                    </motion.div>
                  ) : (
                ...
                <motion.div key="account" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  <h1 className="text-3xl font-black mb-8 flex items-center gap-3">Account <span className="gradient-text">Details</span></h1>
                  <div className="glass-dark rounded-3xl p-8 border border-white/10 shadow-2xl space-y-8">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="p-6 rounded-2xl bg-white/5 border border-white/5">
                           <p className="text-xs text-gray-500 uppercase font-bold tracking-widest mb-2">Display Name</p>
                           <p className="text-xl font-bold text-white">{user.name}</p>
                        </div>
                        <div className="p-6 rounded-2xl bg-white/5 border border-white/5">
                           <p className="text-xs text-gray-500 uppercase font-bold tracking-widest mb-2">Email Address</p>
                           <p className="text-xl font-bold text-white">{user.email}</p>
                        </div>
                        <div className="p-6 rounded-2xl bg-white/5 border border-white/5">
                           <p className="text-xs text-gray-500 uppercase font-bold tracking-widest mb-2">Account Role</p>
                           <p className="text-xl font-bold text-cyan-400 uppercase">{user.role}</p>
                        </div>
                        <div className="p-6 rounded-2xl bg-white/5 border border-white/5">
                           <p className="text-xs text-gray-500 uppercase font-bold tracking-widest mb-2">Joined Date</p>
                           <p className="text-xl font-bold text-white">{new Date(user.createdAt || Date.now()).toLocaleDateString()}</p>
                        </div>
                     </div>
                     
                     <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row gap-4">
                        <button className="btn-secondary px-8 py-3">Update Profile</button>
                        <button onClick={logout} className="btn-primary !bg-red-600/20 !border-red-600/30 !text-red-400 hover:!bg-red-600/40 px-8 py-3">Sign Out</button>
                     </div>
                  </div>
                </motion.div>
              )}
