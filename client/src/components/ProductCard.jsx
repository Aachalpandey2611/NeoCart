  ...
  return (
    ...
    >
      <Link to={`/products/${product._id}`} className="block">
        <div className="glass rounded-2xl overflow-hidden neon-border hover:border-purple-500/50 transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-purple-900/30 scanline-hover panel-pulse">
          {/* Image */}
          <div className="relative overflow-hidden aspect-square bg-gradient-to-br from-purple-950/40 to-blue-950/40">
            {product.badge && (
              <div
                className={`absolute top-3 left-3 z-10 px-2.5 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${badgeColors[product.badge] || "from-purple-500 to-blue-500"} text-white shadow-lg`}
              >
                {product.badge}
              </div>
            )}
            ...
            <motion.img
              src={
                imgError
                  ? `https://placehold.co/400x400/1a1a2e/7c3aed?text=${encodeURIComponent(product.name)}`
                  : product.image
              }
              alt={product.name}
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-4 group-hover:translate-y-0">
              ...
              <motion.div
                whileHover={{ scale: 1.1 }}
                className="p-2.5 glass rounded-xl text-white hover:text-cyan-300 transition-colors"
              >
          ...
          <div className="p-4" style={{ transform: "translateZ(10px)" }}>
            ...
            <div className="flex items-center gap-1 mb-3">
              {[...Array(5)].map((_, i) => (
                <FiStar
                  key={i}
                  className={`w-3 h-3 ${i < Math.floor(product.rating || 0) ? "text-yellow-400 fill-current" : "text-gray-600"}`}
                />
              ))}
